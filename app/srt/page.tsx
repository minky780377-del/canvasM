      ...
      <div className="max-w-5xl mx-auto space-y-6 sm:space-y-10">
        ...
        <header className={`flex flex-col sm:flex-row items-center justify-between border-b ${theme === 'dark' ? 'border-zinc-800' : 'border-zinc-200'} pb-4 sm:pb-4 pt-2 sm:pt-4 gap-3 sm:gap-4`}>
          <div className="flex items-start gap-1.5 sm:gap-2 w-full sm:w-auto justify-start">
            <Link href="/" className={`w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl ${theme === 'dark' ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200 shadow-[0_4px_12px_rgba(0,0,0,0.08)]'} border flex items-center justify-center p-2 sm:p-2.5 shrink-0 -mt-1 sm:mt-0 hover:border-pink-500/50 transition-all active:scale-95`}>
              <Logo className="w-full h-full text-pink-500" />
            </Link>
            <div className="flex flex-col justify-start overflow-hidden pt-1 sm:pt-3.5">
              <div className="flex items-center gap-2">
                <h1 className={`text-xl sm:text-2xl md:text-3xl font-black tracking-tight font-display uppercase leading-none whitespace-nowrap ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>
                  canvasM <span className="text-pink-500">SRT PRO</span>
                </h1>
                <span className="bg-pink-100 text-pink-600 text-[8px] sm:text-[10px] font-black px-2 py-0.5 rounded-full uppercase shrink-0 shadow-sm border border-pink-200">Advanced</span>
              </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto justify-end">
            ...
            <button
              ...
              disabled={isLoading}
              className="flex items-center gap-2 px-4 py-2.5 bg-pink-500 text-white rounded-xl hover:bg-pink-600 transition-all shadow-[0_4px_14px_0_rgba(236,72,153,0.39)] hover:shadow-[0_6px_20px_rgba(236,72,153,0.23)] active:scale-95 disabled:opacity-50 font-bold text-sm"
            >
          ...
          <div className={`flex border-b ${theme === 'dark' ? 'border-zinc-800 bg-zinc-950/50' : 'border-zinc-200 bg-zinc-50/50'} p-2 gap-2`}>
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl text-sm font-bold transition-all ${
                activeTab === 'editor' 
                  ? theme === 'dark' ? 'bg-zinc-800 text-pink-400 shadow-sm' : 'bg-white text-pink-600 shadow-[0_4px_12px_rgba(0,0,0,0.08)] border border-zinc-100' 
              ...
              {activeTab === 'editor' ? (
                ...
                >
                  <div className={`p-6 border-b ${theme === 'dark' ? 'border-zinc-800 bg-zinc-900' : 'border-zinc-200 bg-white'} flex items-center justify-between sticky top-0 z-10`}>
                    ...
                    <button
                      onClick={exportToSRT}
                      className="flex items-center gap-2 px-6 py-2.5 bg-pink-500 text-white rounded-2xl hover:bg-pink-600 transition-all font-bold text-sm shadow-[0_4px_14px_0_rgba(236,72,153,0.39)] active:scale-95"
                    >
                      <Download size={16} />
                      SRT 파일 내보내기
                    </button>
                    ...
                    {blocks.length === 0 ? (
                      ...
                      blocks.map((block, index) => (
                        <motion.div
                          layout
                          key={block.id}
                          className={`${theme === 'dark' ? 'bg-zinc-900 border-zinc-800 shadow-[0_4px_20px_rgba(0,0,0,0.2)]' : 'bg-white border-zinc-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)]'} p-5 rounded-3xl border hover:shadow-lg hover:border-pink-200 transition-all group`}
                        >
                          <div className="flex flex-col sm:flex-row gap-4 items-start">
                            <div className="flex items-center gap-2">
                              <span className={`w-8 h-8 rounded-full ${theme === 'dark' ? 'bg-zinc-800 text-zinc-500' : 'bg-pink-50 text-pink-500 border border-pink-100'} flex items-center justify-center text-xs font-black`}>
                                {index + 1}
                              </span>
                            </div>
                            ...
                            <div className="flex-1 space-y-3 w-full">
                              ...
                              <textarea
                                ...
                                className={`w-full ${theme === 'dark' ? 'bg-zinc-950/50 text-zinc-300 focus:bg-zinc-950' : 'bg-zinc-50 text-zinc-900 focus:bg-white focus:shadow-md'} p-3 rounded-2xl text-sm focus:ring-4 focus:ring-pink-500/10 transition-all border border-transparent focus:border-pink-200 resize-none font-medium`}
                                rows={2}
                              />
                            </div>
                            ...
                            <div className="flex sm:flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button
                                ...
                                className={`p-2 ${theme === 'dark' ? 'text-zinc-500 hover:text-pink-400 hover:bg-pink-400/10' : 'text-zinc-400 hover:text-pink-500 hover:bg-pink-50'} rounded-xl transition-all`}
                                title="아래에 블록 추가"
                              >
                                <Plus size={18} />
                              </button>
              ...
              >
                <div className={`p-8 border-b ${theme === 'dark' ? 'border-zinc-800 bg-zinc-950/50' : 'border-zinc-200 bg-zinc-50/50'} flex items-center justify-between`}>
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-2xl bg-pink-500 flex items-center justify-center text-white shadow-lg shadow-pink-200">
                      <HelpCircle size={24} />
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-8">
                  <section className="space-y-4">
                    <div className="flex items-center gap-3 text-pink-600">
                      <span className="w-6 h-6 rounded-full bg-pink-100 flex items-center justify-center text-xs font-black">1</span>
                  ...
                  <section className="space-y-4">
                    <div className="flex items-center gap-3 text-pink-600">
                      <span className="w-6 h-6 rounded-full bg-pink-100 flex items-center justify-center text-xs font-bold">2</span>
                    ...
                    <p className={`text-sm ${theme === 'dark' ? 'text-zinc-400' : 'text-zinc-600'} leading-relaxed pl-9`}>
                      ...
                      여기서 시작 시간과 종료 시간을 <code className={`${theme === 'dark' ? 'bg-zinc-800 text-pink-400' : 'bg-zinc-100 text-pink-500'} px-1.5 py-0.5 rounded font-mono`}>00:00:00,000</code> 형식에 맞춰 직접 수정하세요.
                    </p>
                  </section>
          ...
          {status && (
            <motion.div
              ...
              className={`fixed top-6 right-6 z-[60] flex items-center gap-2 px-4 py-3 rounded-2xl shadow-2xl border ${
                status.type === 'success' ? 'bg-pink-50 border-pink-100 text-pink-700' : 'bg-pink-50 border-pink-100 text-pink-700'
              }`}
            >
